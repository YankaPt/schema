...
const format = {
    ...
    name: (text) => text.replace(/\w+(.json)/g, '').replace('/', '').replace(".", ""),
    type: (name) => name.replace('#/definitions/', ''),
    hyperlink: (name) => name.replace('/definitions/', ''),
...
const describe = {
    description: (value) => {
        return value.description +
            ...
            (value.default !== undefined ? (value.default !== '' ? '. Default is `' + value.default + '`' : '. Default is empty') : '');
    },
    type: (name, value) => {
        let type = value.type;
        if (type === 'array') {
        ...
        if (value.$ref && value.$ref.indexOf('http') !== 0) {
            ...
            const name = format.name(typeName);
            const file = format.hyperlink(value.$ref.replace('.json', '.md'));
            type = `[${name}](${file})`;
        }
        return type;
    },
...
const tree = {
    ...
    visit: (name, node) => {
        ...
        if (node.oneOf) {
            tree.types.push({ name: name, node: node });
            Object.keys(node.oneOf).forEach((name) => {
                tree.visit(name, node.oneOf[name]);
            });
        }
        if (node.$ref) {
    ...
    node: (nodeName, node) => {
        ...
        if (node.properties) {
            ...
            tree.doc.push('');
            var types = node.oneOf.map(function(type) {
                return type.$ref ? format.type(type.$ref) + ' ' : type.items.$ref ? format.type(type.items.$ref) + '[]' : ':('
            });
            const details = 'Acceptable types: '+ types;
            tree.doc.push(details);
        }
    },
    document: (name, node) => {
...
module.exports.generate = function (file) {
    ...
    if (schema.definitions.object)
        tree.visit(undefined, schema.definitions.object);
    else
        tree.visit(undefined, schema);
    tree.visited_refs = [];
    tree.doc.push('version ' + schema.$version);
    ...
    if (schema.definitions.object) {
        example = tree.example(undefined, schema.definitions.object);
    } else {
        example = tree.example(undefined, schema);
    }
    if (example!=='') {
